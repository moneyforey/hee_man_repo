...
const AddQuestion = (props) => {
  ...
  //const [langvage, setlangvage] = useState("");
  ...
  const APi = async () => {
    try {
      const Responce = await fetch(
        "https://brainbucks.co.in/api/admin/get-subjects"
      );
  ...
  const Api1 = () => {
    ...
    fetch("https://brainbucks.co.in/api/admin/upload-questions", requestOptions)
      ...
      .then((result) => {
        console.log(result);
        const { data, total_questions, unique_questions, repeated_questions } =
          result;
        setQue(data);
        setTotalQuestions(total_questions);
        setRepaetedQuestions(repeated_questions);
        setUniqueQuestion(unique_questions);
        // setOpt(JSON.parse(result.data.options));
  ...
  return (
    <>
        ...
        <div style={{ display: first === 0 ? "block" : "none" }}>
          <div className="mb-5">
            ...
            <p style={{ color: "#8A8A8A" }}>
              0/4 <span sytle={{ color: "red" }}>Steps Completed</span>
            </p>
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    type="search"
                    onChange={(e) => updateResultSubject(e.target.value)}
                  />
                  <br />
                </div>
                <div
                    ...
                    return (
                      <>
                        <div className="d-flex mt-3" key={key}>
                          ...
                          />
                          <span className="ms-3">{res.subject_name}</span>
                        </div>
                      </>
                    );
                  })}
                ...
                <p>
                  {fileName ? (
                    ...
                    <a
                      ...
                      className="mb-2"
                      target="new"
                      href="https://view.officeapps.live.com/op/view.aspx?src=http%3A%2F%2Fbrainbucks.co.in%2Fpublic%2Fbbweb%2Fsample_questions_format.xlsx&wdOrigin=BROWSELINK"
                    >
          ...
          <div className="row">
            ...
            <div className="col-6 d-flex">
              ...
              <p className="col-11" style={{ marginLeft: "-40px" }}>
                Unique Questions
                <span style={{ color: "#367CFF" }}> {unique_quesions}</span>
              </p>
            </div>
            <div className="col-4"></div>
            <div className="col-6 d-flex">
      ...
      <div style={{ display: first === 6 ? "block" : "none" }}>
        ...
        {/* {
           JSON.stringify(gets).map((res,key)=>{
              return(
                <>
                <div key={key}> */}
        <div className="mt-3">
        ...
        {/* {
                 opt.map((res,ind)=>(
                    <InputDiv key={ind} res={res} req={ind} handleChange={handleChangeOptions}/>
                 ))
            }  */}
        {/* {
                    aOpt.map((res,key)=>{
                      return(
                        <>
                        <span className='mt-3 d-flex' style={{justifyContent:"space-evenly"}} key={key}>
                        <input className='col-1' type="radio" style={{width:"20px"}} name="select"/>
                        <span className='col-1' style={{fontSize:20,fontWeight:"600"}}> a:</span>
                        <input className='col-10 border-0' style={{marginLeft:"-30px",}} value={res} onChange={(e)=>{ }} />
                        </span>
                        </>
                      )
                    })
                  } */}
        {/* <span className=' mt-3 d-flex' style={{justifyContent:"space-evenly"}}>
                  <input className='col-1' type="radio" style={{width:"20px"}} name="select"/>
                  <span className='col-1' style={{fontSize:20,fontWeight:"600"}}> b:</span>
                  <input className='col-10 border-0' style={{marginLeft:"-30px"}} value={Opb} onChange={(e)=>{setOpb(e.target.value)}}   placeholder={Question}/>
                  </span>
                  <span className='mt-3 d-flex' style={{justifyContent:"space-evenly"}}>
                  <input className='col-1' type="radio" style={{width:"20px"}} name="select"/>
                  <span className='col-1' style={{fontSize:20,fontWeight:"600"}}> c:</span>
                  <input className='col-10 border-0' style={{marginLeft:"-30px"}} value={Opc} onChange={(e)=>{setOpc(e.target.value)}}   placeholder={Question}/>
                  </span>
                  <span className='mt-3 d-flex' style={{justifyContent:"space-evenly"}}>
                  <input className='col-1' type="radio" style={{width:"20px"}} name="select"/>
                  <span className='col-1' style={{fontSize:20,fontWeight:"600"}}> d:</span>
                  <input className='col-10 border-0' style={{marginLeft:"-30px"}} value={Opd} onChange={(e)=>{setOpd(e.target.value)}}   placeholder={Question}/>
                  </span> */}
        {/* </div> 
                 </>
              )
